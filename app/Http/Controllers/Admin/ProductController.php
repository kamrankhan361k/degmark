...
class ProductController extends Controller
{
    ...
    public function store(Request $request){
        ...
        $rules = [
            ...
            'author'=> $setting->vendor_type == 'multi' ? 'required' : '',
            'category'=>'required',
            'name'=>'required',
        ...
        if($setting->vendor_type == 'single'){
            $product->admin_id = $auth_admin->id;
        }
        $product->product_type = $request->product_type;
        $product->author_id = $setting->vendor_type == 'multi' ? $request->author : 0;
